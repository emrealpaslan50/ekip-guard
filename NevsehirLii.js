...
const fayikik1 = require('./NevsehirLii.json')
...
const fayikik2 = require('./NevsehirLii.json')
...
**Yetkili sunucudan yasaklandı! Rollerdeki tüm yetkiler kapatıldı.**`)
channel.send({embed: fayik})
return client2.users.cache.get(ayarlar2.sahip).send(`__**Sunucuda rol açıldı!__ açan kişinin bilgileri :** \n**Kullanıcı Adıı :** ${entry.executor.tag} **Kullanıcı İdsi :** ${entry.executor.id}`)
});
...
**Yetkili sunucudan yasaklandı! Rollerdeki tüm yetkiler kapatıldı.**`)
  ...
  return client2.users.cache.get(ayarlar2.sahip).send(`**Sunucuda rol güncellendi! Günceliyen kişinin bilgileri :** \n**Kullanıcı Adı :** ${entry.executor.tag} **Kullanıcı İdsi :** ${entry.executor.id}\n**Rol Adı :**${oldRole.name}**Rol İdsi :** ${oldRole.id}`)
    ...
    const yetkiPermleri = ["MANAGE_ROLES", "MANAGE_CHANNELS", "MANAGE_GUILD", "BAN_MEMBERS", "KICK_MEMBERS", "MANAGE_NICKNAMES", "MANAGE_EMOJIS", "MANAGE_WEBHOOKS"];
...
const fayikik3 = require('./NevsehirLii.json')
...
let channel2 = client3.channels.cache.get(ayarlar3.log3)
...
client3.on("channelUpdate", async (oldChannel, newChannel) => {
  ...
  let channel = client3.channels.cache.get(ayarlar3.log3)
...
let sesKanal = client1.channels.cache.get(ayarlar1.botses);
  if (sesKanal)
    sesKanal
      .join()
...
console.log(`${client1.user.username} ismi ile giriş yapıldı! Guard I Online`);
  client1.channels.cache.get(ayarlar1.botkanal).send(`${client1.user.username} \`ismi ile giriş yapıldı!\``)
  
  });

client1.login(ayarlar1.bot1).catch(err => {
...
let sesKanal = client2.channels.cache.get(ayarlar2.botses);
  if (sesKanal)
    sesKanal
      .join()
    .catch(err => console.error("Bot ses kanalına bağlanamadı!"));
...
console.log(`${client2.user.username} ismi ile giriş yapıldı! Guard II Online`);
  client2.channels.cache.get(ayarlar2.botkanal).send(`${client2.user.username} \`ismi ile giriş yapıldı!\``)
  
  });
client2.login(ayarlar2.bot2).catch(err => {
...
client3.user.setPresence({ activity: { name: ayarlar1.botdurum }, status: ayarlar3.status2 });
let sesKanal = client3.channels.cache.get(ayarlar3.botses);
  if (sesKanal)
    sesKanal
      .join()
      .catch(err => console.error("Bot ses kanalına bağlanamadı!"));
...
console.log(`${client3.user.username} ismi ile giriş yapıldı! Guard III Online`);
     client3.channels.cache.get(ayarlar3.botkanal).send(`${client3.user.username} \`ismi ile giriş yapıldı!\``)
  
});
client3.login(ayarlar3.bot3).catch(err => {
